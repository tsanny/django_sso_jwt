...
from .utils import get_cas_client, get_service_url, get_protocol
...
class LoginView(View):
    ...
    def successful_login(self, request: HttpRequest, next_page: str) -> HttpResponse:
        ...
        user_details_serialized = request.COOKIES.get('user', None)
        if user_details_serialized:
            details = json.loads(user_details_serialized)
            response = JsonResponse({
                'user': details, 'tokens': tokens
            })
            return response
        ...
        # Load organization details from a file
        with open(settings.SSO_UI_ORG_DETAIL_FILE_PATH, 'r') as file:
            org_detail = json.load(file)
        user_org = org_detail['id'].get(attr.get('kd_org', ''), {})
        details.update(user_org)

        response = JsonResponse({
            'user': details, 'tokens': tokens
        })

        user_details_serialized = json.dumps(details)
        response.set_cookie('user', user_details_serialized, httponly=True)
